...
def step_by_step_dao(client: AzureOpenAI, directory_path: str, is_handcrafted: bool, model: str, max_tokens: int):
    """
        ...
        for idx, entry in enumerate(chat_history):
            ...
            if "yes" in prefix_area:
                ...
                dao_member_prompt_1 = (
                        ...
                        f"An Accuser claims that Step {idx} by '{agent_name}' is critically flawed for the following reason:\n"
                        f"\"{reason}\"\n"
                        "Do you agree that this step contains a fatal error that leads to task failure? "
                        "Respond ONLY with 'VOTE: GUILTY' if you agree the step is fatally flawed, or 'VOTE: INNOCENT' if not."
                    )
                dao_member_prompt_2 = (
                        f"You are an impartial Arbitrator  reviewing an accusation in a multi-agent system.\n"
                        f"Problem: {problem}\nAnswer: {ground_truth}\n"
                ...
                for i,dao_member_prompt in enumerate(dao_member_prompts):

                    dao_messages = [{"role": "system", "content": sys_prompts[i]}, {"role": "user", "content": dao_member_prompt}]
                    dao_answer = _make_api_call(client, model, dao_messages, max_tokens)
                    if dao_answer and "guilty" in dao_answer.lower():
                        dao_votes_guilty += 1
                        print(f"Arbitrator voted: GUILTY")
                    else:
                        print(f"Arbitrator voted: INNOCENT")
                
                print(f"DAO Arbitration closed. Result: {dao_votes_guilty}/3 GUILTY votes.")
                
                if dao_votes_guilty >= 2:
                    print(f"\nPrediction for {json_file}: Error found.")
                    print(f"Agent Name: {agent_name}")
                    print(f"Step Number: {idx}")
                    ...
                    print("Accusation rejected (Not enough consensus). Proceeding to next step.")
